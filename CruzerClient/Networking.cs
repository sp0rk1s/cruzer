...
namespace CruzerClient {
	class Networking {
		...
		public static Exception? PingNetwork(IPAddress iPAddress, UInt16 port = 11111, IPEndPoint? localIPEndPoint = null) {
			...
			// Pinging
			new Packet(new PacketHeader(0, 0, 0, PacketType.Ping, true, true), []).Send(socket);
			Packet packet = new(socket);
			if (packet.Header.Type == PacketType.Ping) {
				return null;
			}
			return new Exception("Bad server");
		}
